          ...
          <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
            <div className="p-4">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold">{gameType === "chess" ? "Chess" : "Tic-Tac-Toe"}</h2>
...
function ChessGame({ matchId }) {
  ...
  const renderStatusMessage = () => {
    switch (gameStatus) {
      ...
      case "checkmate":
        return currentTurn === playerColor ? "Checkmate! You lost." : "Checkmate! You won!"
      case "draw":
        ...
        <div className="rounded-lg border bg-card p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">{renderStatusMessage()}</div>
            </div>
          </div>
        </div>
      </div>
...
function TicTacToeGame({ matchId }) {
  ...
  const makeMove = (index) => {
    if (gameStatus !== "playing" || board[index] !== null || currentPlayer !== playerSymbol) {
      return
    }
  ...
  useEffect(() => {
    if (gameStatus === "playing" && currentPlayer !== playerSymbol && !board.every((cell) => cell !== null)) {
      const timer = setTimeout(() => {
        // Find empty cells
      ...
      <div className="mb-4 w-full">
        <div
          className={`rounded-lg border bg-card p-4 ${
            gameStatus === "won" ? "border-green-500" : gameStatus === "lost" ? "border-destructive" : ""
          }`}
        >
          <div className="flex items-center gap-2">
            <svg
  ...
  return (
    <div className="rounded-lg border bg-card text-card-foreground shadow-sm h-full flex flex-col">
      <div className="flex flex-col space-y-1.5 p-6 pb-3">
        <h3 className="text-lg font-semibold leading-none tracking-tight">Game Chat</h3>
      </div>
        ...
        <div className="h-[400px] px-4 overflow-y-auto">
          <div className="space-y-4 pt-1">
            {messages.map((message) => (
              <div
                key={message.id}
                className={`flex ${message.sender === "System" ? "justify-center" : message.sender === user?.username ? "justify-end" : "justify-start"}`}
              >
                {message.sender !== "System" && message.sender !== user?.username && (
