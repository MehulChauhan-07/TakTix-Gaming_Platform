...
function handleTicTacToeEvents(io, socket, match) {
        ...
        try {
            ...
            if (gameOver) {
                ...
                if (winner) {
                    ...
                    // Update user stats
                    await (0, userService_1.updateUserStats)(winner, true);
                    await (0, userService_1.updateUserStats)(match.players.find((p) => p.user.toString() !== winner)?.user.toString() || "", false);
                }
                else if (isDraw) {
                    // Update both players for a draw
                    for (const player of match.players) {
                        player.score = 0.5;
                        await (0, userService_1.updateUserStats)(player.user.toString(), null);
                    }
                }
            }
            await match.save();
    ...
    socket.on("forfeit", async () => {
        try {
            ...
            // Update stats
            if (winner) {
                await (0, userService_1.updateUserStats)(winner.user.toString(), true);
                await (0, userService_1.updateUserStats)(socket.userId, false);
            }
