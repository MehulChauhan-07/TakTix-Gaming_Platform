...
export class TicTacToe {
  ...
  makeMove(board: (string | null)[], move: { position: number; player: string }) {
  ...
  isValidMove(board: (string | null)[], position: number): boolean {
    return position >= 0 && position < this.BOARD_SIZE && board[position] === null;
  }

  getAvailableMoves(board: (string | null)[]): number[] {
    return board
